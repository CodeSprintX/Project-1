...
export function CartProvider({children} : {children : ReactNode}) {
    ...
    const fetchCart = async()=>{
        setIsLoading(true);
        const serveCart = dummyCart;
        const mappedItems : CartItem[] = serveCart.items.map((item : any)=>({
            id : item.product._id,
        ...
        setCartTotal(serveCart.totalAmount)
        setIsLoading(false)
    }
   ...
   const clearCart = async(productId : string, quantity:number, size : string = 'M')=>{
